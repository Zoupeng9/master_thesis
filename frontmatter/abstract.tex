...
实际地球介质是非弹性的，当地震波在地层中传播时将引起衰减，衰减强度通常
用品质因子$Q$来描述。强衰减往往出现在油气勘探的目标区域如气云、
气包（gas clouds/pockets）区。非弹性衰减对地震波的运动学和动力学特征都会
产生影响：速度频散和振幅衰减。
在地震数据处理中，如果不考虑地震衰减效应将直接影响地震解释和储层识别的可靠性。
在偏移成像中可以通过衰减补偿来校正速度频散和能量损失，所有的带
衰减补偿的偏移算法均需要相对准确的宏观背景$Q$模型。

$Q$模型估计一直是地震学领域的研究热点之一。早期用衰减前后地震波
的振幅谱的比值来构建层析方程，为了消除非衰减因素对振幅谱的影响，学者们通过检测
质心频率的变化来反演$Q$值。这类方法通过射线追踪计算层析矩阵，当介质变化复杂时
射线理论存在焦散以及多路径效应，从而降低了反演的精度。
随着高性能计算机技术的不断进步以及宽频带、宽方位、高密度地震数据采集技术的逐渐
成熟，单参数(速度)的全波形反演（FWI）技术逐渐走向成熟。近年来，粘声介质的全波形
反演（$Q$-FWI）技术也得到了飞速发展。$Q$-FWI以波动方程为传播引擎，能适应复杂地质
情况，但其利用的主要是透射波信息。当缺乏低频、长
偏移距数据时，透射波对地下的穿透深度有限，$Q$-FWI只能反演得到浅层的模型参数。
为了获得中深部的$Q$模型，有学者近年来提出了
成像域偏移$Q$分析方法。现有的研究结果表明成像域方法反演结果的分辨率一般比数据域方法低。

本文第2章系统总结了地震衰减的基本理论，并着重讨论了衰减介质$Q$补偿逆时偏移（$Q$-RTM）方法。
为了获取$Q$-RTM所需的中深层宏观$Q$模型，论文
第3章首次在反射波形反演（$Q$-RWI）框架下反演宏观$Q$模型。当观测地震数据缺少大孔径的
透射波信息时，$Q$-RWI利用反射波数据更新中深层$Q$模型。$Q$-RWI强烈依赖速度
模型，包括宏观背景速度和高波数速度扰动。在实际勘探中光滑的背景速度相对容易获取，
在缺乏$Q$信息的情况下，即使背景速度模型足够准确，最小平方逆时偏移（LSRTM）也很难
反演出准确的高波数速度模型。
为了降低$Q$-RWI对速度模型高波数成分的依赖性，论文第4章引入了峰值频率
移动目标函数。峰值频率是地震数据的一个属性，其大小只与地震子波
以及地层的衰减有关。
只要速度的高波数成分不影响地震子波的形状就不会改变峰值频率的大小。
由于反射地震数据峰值频率信息与高波数速度成分的大小、极性均无关系，
在实际反演中用任意两层速度模型与其平滑模型的差在反射界面位置处代替高波数
速度扰动即可满足反演的要求。

本文针对中深层背景$Q$反演遇到的问题，提出了$Q$-RWI反演方法。通过峰值频率移动
目标函数的引入降低了$Q$-RWI对高波数速度模型的依赖，使反演方法更加
实用化。
...
peak frequencies shifts misfit function and make it more practical.
  ...
  \englishkeywords{\zihao{-4}  
  	Attenuation, \quad 
  	Visco-acoustic media, \quad
    Reflection waveform inversion, \quad
	Peak frequancy, \quad
	$Q$ migration
  }
