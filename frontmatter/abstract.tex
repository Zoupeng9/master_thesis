...
成像域中的偏移$Q$分析方法。现有的研究结果表明成像域方法反演结果的分辨率一般比数据域方法低。

本文第2章系统总结了地震衰减的基本理论，并着重讨论了衰减介质$Q$补偿的逆时偏移（$Q$-RTM）方法。
为了获取$Q$-RTM所需的中深层宏观$Q$模型，论文
第3章首次将$Q$反演纳入反射波形反演（$Q$-RWI）框架。当观测地震数据缺少大孔径的
透射波信息时，$Q$-RWI利用反射波数据更新中深层$Q$模型。$Q$-RWI强烈依赖速度
模型，包括宏观背景速度和高波数速度扰动。在实际勘探中背景速度相对容易获取，
